...
func (b *IconButton) CreateRenderer() fyne.WidgetRenderer {
	b.iconImage = canvas.NewImageFromResource(b.icon)
	b.iconImage.FillMode = canvas.ImageFillContain
	b.iconImage.SetMinSize(fyne.NewSize(16, 16))

	b.bg.SetMinSize(b.size)

	var content fyne.CanvasObject

	// Eğer text varsa, icon ve text'i yatay olarak düzenle
	if b.text != "" {
		b.textLabel = widget.NewLabel(b.text)
		content = container.NewHBox(
			container.NewStack(b.bg, b.iconImage),
			b.textLabel,
		)
	} else {
		// Sadece icon
		content = container.NewStack(b.bg, b.iconImage)
	}
